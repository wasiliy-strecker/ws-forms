    ...
    <?php foreach ($users as $index => $user): ?>
        ...
        <li class="wsf_m3_list_item" itemprop="itemListElement" itemscope itemtype="https://schema.org/Person">
            ...
            <div class="wsf_m3_trailing ">
                ...
                <?php endif; ?>
                <?php
                    $firstName = !empty($user->wsf_first_name) ? sanitize_title($user->wsf_first_name) : 'user';
                    $lastName = !empty($user->wsf_last_name) ? sanitize_title($user->wsf_last_name) : 'edit';
                    $prettyUrl = home_url("/ws-forms/{$user->ID}-{$firstName}-{$lastName}/");
                ?>
                <a href="<?php echo $prettyUrl; ?>"
